...
public class Quadrado{
    ...
    public ArrayList<Shape> espelhar(){
        ...
        Double[][] espelhar = {{-1.0,0.0},
                            {0.0,1.0}};
        for (int i = 0; i < this.v; i++) {
            Double aux = (p[i][0] * espelhar[0][0]) + (p[i][1] * espelhar[0][1]);
            Double aux1 = (p[i][0] * espelhar[1][0]) + (p[i][1] * espelhar[1][1]);
            this.p[i][0] = aux+60;
            this.p[i][1] = aux1;
            points[i] = new Point2D.Double(p[i][0], p[i][1]);
        }
    ...
    public ArrayList<Shape> girar(Double x){
        sps =new ArrayList<Shape>();
        Double[] girar = {Math.cos(x), - Math.sin(x)
                            ,Math.sin(x),   Math.cos(x)};
        for (int i = 0; i < this.v; i++) {
            Double aux = (p[i][0] * girar[0]) + (p[i][1] * girar[1]);
            Double aux1 = (p[i][0] * girar[2]) + (p[i][1] * girar[3]);
            this.p[i][0] = aux;
            this.p[i][1] = aux1;
            points[i] = new Point2D.Double(p[i][0], p[i][1]);
        }
